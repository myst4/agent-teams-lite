...
These hooks turn two of Kurama's prose rules into **mechanisms** Claude
...
- The target path is under **`.kurama/`** (harness state) or **`openspec/`** (SDD
...
| Variable | Effect |
|----------|--------|
| `KURAMA_ORCHESTRATOR_GUARD=0` | Disable the write guard entirely. |
| `KURAMA_GUARD_BYPASS=1` | Allow a single write past the guard (per-call escape hatch). |
| `KURAMA_ARCHIVE_OVERRIDE=1` | Bypass the archive gate — **both** the verify-PASS gate and the content-binding (stale-receipt) check. Mirrors `sdd-archive` Step 0's user-authorized override — the **reason must still be recorded** in the archive report; the script only opens the gate. |
...
- Set `KURAMA_ORCHESTRATOR_GUARD=0` (write guard) in the environment, **or**
